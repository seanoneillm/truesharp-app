-- Parlay-Aware Strategy Leaderboard Calculation
-- This updated trigger function properly handles parlays as single betting units
-- instead of counting each individual leg separately

-- Create or replace the function that updates strategy totals with parlay awareness
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    SET 
      ...
      total_bets = (
        ...
        SELECT SUM(count) FROM betting_units
      ),
...
-- Parlay-aware bet status change handler
...
-- Replace the old triggers with parlay-aware versions
...
-- Keep the existing updated_at trigger
-- CREATE TRIGGER trigger_strategy_leaderboard_updated_at BEFORE UPDATE ON strategy_leaderboard
--     FOR EACH ROW EXECUTE FUNCTION update_strategy_leaderboard_updated_at();

-- Optional: Function to recalculate all existing leaderboard entries with parlay awareness
...
-- Usage comment:
-- To recalculate all existing entries: SELECT recalculate_all_strategy_leaderboard_with_parlays();
-- Corrected SQL for claimable subscription functions
-- This version uses the correct column names that exist in the current table
-- Execute this in Supabase SQL editor

-- Add new status to pro_subscriptions table if needed
DO $$ 
BEGIN
    -- Update status constraint to include pending_user_validation
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'pro_subscriptions_status_check'
        AND table_name = 'pro_subscriptions'
    ) THEN
        ALTER TABLE pro_subscriptions 
        DROP CONSTRAINT pro_subscriptions_status_check;
    END IF;
  ...
  ELSE
    ...
    ELSE
      ...
      INSERT INTO pro_subscriptions (
        ...
        apple_environment,
        receipt_validation_status,  -- Use correct column name
        ...
        p_original_transaction_id,
        p_environment,
        'pending',  -- Use correct column name
        NOW(),
        0,
        NOW(),
        NOW()
...
BEGIN
  ...
  UPDATE pro_subscriptions SET
    ...
    receipt_validation_status = 'validated',  -- Use correct column name

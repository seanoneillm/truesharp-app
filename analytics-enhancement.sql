...
-- 2) Helper: normalized net_profit: use profit if present, otherwise infer
CREATE OR REPLACE FUNCTION analytics.net_profit_for_row(b bets)
RETURNS numeric 
LANGUAGE sql 
STABLE 
AS $$
  SELECT COALESCE(b.profit,
                  CASE
...
-- 3) Add index on league for performance (if not present)
CREATE INDEX IF NOT EXISTS idx_bets_league ON public.bets USING btree (league);
...
-- Input: filters jsonb with optional keys: sports, leagues, bet_types, sportsbooks, player_names, odds_min, odds_max, stake_min, stake_max, date_from, date_to, is_parlay, prop_type, side
-- x_dim: 'date:day', 'date:week', 'date:month', 'league', 'bet_type', 'odds_bucket', 'stake_bucket', 'day_of_week', 'player_name', etc.
-- y_metric: 'roi', 'net_profit', 'win_rate', 'total_stake', 'count', 'avg_stake'
    ...
    _sql := format('
      SELECT to_jsonb(t) FROM (
        SELECT date_trunc(%L, placed_at) as x,
               sum(COALESCE(profit, potential_payout - stake, CASE WHEN status=''lost'' THEN -stake ELSE 0 END)) as net_profit,
               sum(stake) as stake,
               CASE WHEN sum(stake) = 0 THEN NULL ELSE (sum(COALESCE(profit, potential_payout - stake, CASE WHEN status=''lost'' THEN -stake ELSE 0 END))/sum(stake))*100 END as roi_pct,
               count(*) as bets
        FROM bets
        %s
        GROUP BY 1
...
-- 6) ROI Over Time RPC (daily buckets, moving averages computed client-side or via SQL)
...
-- 7) Performance by League (replace sport -> league). Only show leagues with >= 10 bets.
...
WITH f AS (
  ...
  WHERE user_id = p_user
    ...
    AND (NOT (p_filters ? 'bet_types') OR bet_type = ANY (SELECT jsonb_array_elements_text(p_filters->'bet_types')::text[]))
    AND (NOT (p_filters ? 'sports') OR sport = ANY (SELECT jsonb_array_elements_text(p_filters->'sports')::text[]))
    AND (NOT (p_filters ? 'sportsbooks') OR sportsbook = ANY (SELECT jsonb_array_elements_text(p_filters->'sportsbooks')::text[]))
    AND (NOT (p_filters ? 'odds_min') OR odds >= (p_filters->>'odds_min')::integer)
    AND (NOT (p_filters ? 'odds_max') OR odds <= (p_filters->>'odds_max')::integer)
    AND (NOT (p_filters ? 'stake_min') OR stake >= (p_filters->>'stake_min')::numeric)
...
-- 12) Security notes and RLS setup (if needed)
-- Consider using RLS on public.bets to ensure functions only return rows for auth.uid()
-- If using SECURITY DEFINER functions, explicitly verify that p_user = auth.uid() or restrict to superuser usage
...
-- 13) Additional utility functions for odds conversion
...
-- 14) Bucket helpers for chart building
...
-- Create sample refresh trigger (optional - can be called manually or via cron)
-- REFRESH MATERIALIZED VIEW analytics.mv_daily_user_perf;
-- REFRESH MATERIALIZED VIEW analytics.mv_monthly_user_perf; 
-- REFRESH MATERIALIZED VIEW analytics.mv_league_user_perf;
...
COMMENT ON FUNCTION analytics.net_profit_for_row(bets) IS 'Calculates normalized net profit for a bet row';

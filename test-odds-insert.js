...
async function insertOdds(normalizedOdds) {
  ...
  for (let i = 0; i < normalizedOdds.length; i += batchSize) {
    ...
    console.log(
      `🔄 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(normalizedOdds.length / batchSize)} (${batch.length} odds)`
    )
    ...
    for (const odd of batch) {
      try {
        ...
        if (existingOdds && existingOdds.length >= 2) {
          ...
          // Insert new odd
          const { error: insertError } = await supabase.from('odds').insert(oddData)

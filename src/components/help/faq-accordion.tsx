...
const faqItems = [
  ...
  },
  {
    ...
    answer: "Yes. TrueSharp never stores your sportsbook credentials. All syncing is handled securely through SharpSports."
  },
  {
    question: "How often do my bets update?",
    answer: "Bets update automatically, but you can always click Refresh Bets in Analytics to fetch the latest."
  }
]
    ...
    <div className="space-y-3">
      {faqItems.map((item, index) => (
        <Collapsible key={index} open={openItems.has(index)} onOpenChange={() => toggleItem(index)}>
          <CollapsibleTrigger className="flex w-full items-center justify-between rounded-lg bg-gray-50 px-4 py-3 text-left font-medium text-gray-900 hover:bg-gray-100 transition-colors">
            {item.question}

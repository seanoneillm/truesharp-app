...
function getSportBadgeColor(sport: string) {
  switch (sport) {
    ...
    case 'SOCCER': return 'bg-green-100 text-green-800'
...
function getStatusIcon(status: string) {
  switch (status) {
    ...
    case 'void': return <XCircle className="h-4 w-4 text-gray-500 mr-1" />
    case 'cancelled': return <XCircle className="h-4 w-4 text-gray-500 mr-1" />
    default: return null
  }
}
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-2">
          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getSportBadgeColor(sport)}`}>
            {sport}
          </span>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <p className={`text-lg font-semibold ${getPayoutColor(status)}`}>
            {displayPayout}
          </p>

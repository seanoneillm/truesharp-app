...
export default function OddsButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`
        font-medium rounded-full transition-all duration-200 border cursor-pointer text-center shadow-sm hover:shadow-md
        ${getSizeClasses()}
        ${getVariantClasses()}
    ...
    >
      <div className="flex flex-col items-center leading-tight">
        {line !== undefined && (
          <div className="text-xs opacity-80 font-medium leading-none">
            {formatLineDisplay()}
          </div>
        )}
        <div className="font-semibold text-xs sm:text-sm leading-none mt-0.5">
          {formatOdds(odds)}
        </div>
      </div>
    </button>

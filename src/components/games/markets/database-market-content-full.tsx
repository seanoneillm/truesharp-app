    ...
    const fetchDatabaseOdds = async () => {
      try {
        ...
        if (gameWithOdds && gameWithOdds.odds && gameWithOdds.odds.length > 0) {
          ...
          console.log('📈 Sample odds:', gameWithOdds.odds.slice(0, 3).map(odd => ({
            oddid: odd.oddid,
    ...
    return (
      <div className="text-center py-8">
        <div className="p-4 text-center text-slate-500 bg-slate-50 rounded-lg">
          ...
          <div className="text-xs mt-1">Try selecting a different tab</div>
        </div>
      </div>
    );
  }
              ...
              {moneylineOdds.slice(0, 2).map((odd, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 active:bg-blue-800 transition-all duration-200 font-semibold shadow-sm hover:shadow-md transform hover:-translate-y-0.5"
                  >
                    {(() => {
                      const bestOdds = getBestOdds(odd);
              ...
              {spreadOdds.slice(0, 2).map((odd, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 bg-orange-600 text-white rounded-xl hover:bg-orange-700 active:bg-orange-800 transition-all duration-200 font-semibold shadow-sm hover:shadow-md transform hover:-translate-y-0.5"
                  >
        ...
        {totalOdds.length > 0 && (
          <div>
            ...
            {combineOverUnderMarkets(totalOdds).slice(0, 1).map((market, index) => (
              <div key={index} className="grid grid-cols-3 gap-3 items-center p-3 bg-slate-50 rounded-lg">
                <div>
                  <div className="font-medium text-slate-700">Total</div>
                  {market.line && (
                    <div className="text-sm text-slate-500">{market.line}</div>
                  )}
                </div>
                <div className="flex justify-center">
                  {market.over && (
                    <button
                      ...
                      className="px-4 py-2.5 bg-green-600 text-white rounded-xl hover:bg-green-700 active:bg-green-800 transition-all duration-200 font-semibold text-sm shadow-sm hover:shadow-md transform hover:-translate-y-0.5"
                    >
                ...
                <div className="flex justify-center">
                  {market.under && (
                    <button
                      ...
                      className="px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 active:bg-red-800 transition-all duration-200 font-semibold text-sm shadow-sm hover:shadow-md transform hover:-translate-y-0.5"
                    >
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {combinedMarkets.slice(0, 12).map((market, index) => (
          <div key={index} className="grid grid-cols-4 gap-3 items-center py-3 border-b border-slate-100 last:border-b-0">
            ...
            <div className="flex justify-center">
              {market.over ? (
                <button
                  ...
                  className="px-4 py-2 text-sm font-semibold rounded-xl border-2 transition-all duration-200 shadow-sm hover:shadow-lg bg-green-50 text-green-700 border-green-300 hover:bg-green-600 hover:text-white transform hover:-translate-y-0.5 active:scale-95"
                >
            ...
            <div className="flex justify-center">
              {market.under ? (
                <button
                  ...
                  className="px-4 py-2 text-sm font-semibold rounded-xl border-2 transition-all duration-200 shadow-sm hover:shadow-lg bg-red-50 text-red-700 border-red-300 hover:bg-red-600 hover:text-white transform hover:-translate-y-0.5 active:scale-95"
                >

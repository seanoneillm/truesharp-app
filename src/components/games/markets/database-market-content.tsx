...
function DatabaseMarketContent({
  ...
  const combineOverUnderMarkets = (odds: DatabaseOdds[]): { marketName: string; over?: DatabaseOdds; under?: DatabaseOdds; line?: string | number | undefined }[] => {
    const marketGroups: Record<string, { over?: DatabaseOdds; under?: DatabaseOdds; line?: string | number | undefined }> = {};
    ...
    odds.forEach(odd => {
      ...
      const isUnder = odd.sideid?.toLowerCase().includes('under') || odd.sideid?.toLowerCase() === 'u';
  ...
  const renderMainLines = (odds: Record<string, DatabaseOdds[]> = organizedOdds) => {
    ...
    if (mainOdds.length === 0) {
      return (
        <div className="text-center py-8">
          <div className="p-3 text-center text-slate-500 bg-slate-50 rounded-lg">
            <div className="text-sm">No main line odds available</div>
            <div className="text-xs mt-1">Check enhanced odds or try refreshing</div>
          </div>
        </div>
      );
    }
    ...
    const totalOdds = mainOdds.filter(odd => 
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3 text-xs font-semibold text-slate-600 uppercase tracking-wide px-2">
          <div>Market</div>
        ...
        {spreadOdds.length > 0 && (
          <div className="space-y-2">
            ...
            {spreadOdds.slice(0, 2).map((odd, index) => {
              ...
              return (
                <div key={`spread-${index}`} className="grid grid-cols-3 gap-3 items-center">
                  ...
                  <div className="flex justify-center">
                    <button
                      ...
                      className="px-3 py-1.5 text-sm font-medium rounded-full border transition-all duration-200 shadow-sm hover:shadow-md bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-100"
                    >
        ...
        {totalOdds.length > 0 && (
          <div className="space-y-2">
            ...
            {combineOverUnderMarkets(totalOdds).slice(0, 1).map((market, index) => (
              <div key={`total-combined-${index}`} className="grid grid-cols-4 gap-3 items-center">
                <div className="text-sm font-medium text-slate-700">
                  Total
                </div>
                <div className="text-center text-sm text-slate-600">
                ...
                <div className="flex justify-center">
                  {market.over && (
                    <button
                      ...
                      className="px-3 py-1.5 text-sm font-medium rounded-full border transition-all duration-200 shadow-sm hover:shadow-md bg-green-50 text-green-700 border-green-200 hover:bg-green-100"
                    >
                ...
                <div className="flex justify-center">
                  {market.under && (
                    <button
                      ...
                      className="px-3 py-1.5 text-sm font-medium rounded-full border transition-all duration-200 shadow-sm hover:shadow-md bg-red-50 text-red-700 border-red-200 hover:bg-red-100"
                    >
        ...
        {moneylineOdds.length > 0 && (
          <div className="space-y-2">
            ...
            {moneylineOdds.slice(0, 2).map((odd, index) => {
              ...
              return (
                <div key={`moneyline-${index}`} className="grid grid-cols-3 gap-3 items-center">
                  <div className="text-sm font-medium text-slate-700">
                    {formatTeamForDisplay(teamName, 8)}
                  </div>
                  <div className="text-center text-sm text-slate-600">
                  ...
                  <div className="flex justify-center">
                    <button
                      ...
                      className="px-3 py-1.5 text-sm font-medium rounded-full border transition-all duration-200 shadow-sm hover:shadow-md bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100"
                    >
                      {toAmericanOdds(odd.bookodds || 0)}
                    </button>
                  </div>
  ...
  const renderPlayerProps = (odds: Record<string, DatabaseOdds[]> = organizedOdds) => {
      ...
      return (
        <div className="text-center py-8">
          <div className="p-3 text-center text-slate-500 bg-slate-50 rounded-lg">
            ...
            <div className="text-xs mt-1">Check back later for more markets</div>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="text-center py-8">
          <div className="p-3 text-center text-slate-500 bg-slate-50 rounded-lg">
            ...
            <div className="text-xs mt-1">Try selecting a different category</div>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-3 text-xs font-semibold text-slate-600 uppercase tracking-wide px-2">
        ...
        <div className="space-y-2">
          {combinedMarkets.slice(0, 8).map((market, index) => {
            ...
            return (
              <div key={`combined-${index}`} className="grid grid-cols-4 gap-3 items-center py-2 border-b border-slate-100 last:border-b-0">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {combinedMarkets.slice(0, 6).map((market, index) => (
            <div key={`team-combined-${index}`} className="grid grid-cols-4 gap-3 items-center py-2 border-b border-slate-100 last:border-b-0">
              <div className="text-sm">
  ...
  const renderGameProps = (odds: Record<string, DatabaseOdds[]> = organizedOdds) => {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {combinedMarkets.slice(0, 6).map((market, index) => (
            <div key={`game-combined-${index}`} className="grid grid-cols-4 gap-3 items-center py-2 border-b border-slate-100 last:border-b-0">
              <div className="text-sm">
                <div className="font-medium text-slate-800">{market.marketName}</div>
              </div>
              <div className="text-center text-sm text-slate-600">
    ...
    return (
      <div className="text-center py-8">
        <div className="p-4 text-center text-slate-500 bg-slate-50 rounded-lg">
          ...
          <div className="text-sm mt-1">This game may not have betting lines yet</div>
        </div>
      </div>
    );
  }

    ...
    switch (sport) {
      ...
      case 'basketball_ncaab':
        return {
          ...
          'rebounding': { label: 'Rebounding', icon: '↩️', description: 'Total, Offensive, Defensive Rebounds' },
          'playmaking': { label: 'Playmaking', icon: '🎯', description: 'Assists, Turnovers, Steals, Blocks' }
        };
      
      case 'baseball_mlb':
        return {
          'hitters': { label: 'Hitters', icon: '⚾', description: 'Hits, Home Runs, RBIs, Runs' },
          'pitchers': { label: 'Pitchers', icon: '🥎', description: 'Strikeouts, Walks, Innings, ERs' }
        };
      
      case 'icehockey_nhl':
        return {
          'skaters': { label: 'Skaters', icon: '🏒', description: 'Goals, Assists, Points, Shots, Hits' },
      ...
      case 'soccer_usa_mls':
        return {
          'forwards': { label: 'Forwards', icon: '⚽', description: 'Goals, Shots on Target, Assists' },
          'midfielders': { label: 'Midfielders', icon: '🎯', description: 'Passes, Tackles, Assists' },
    ...
    <div className="border-b border-slate-100 bg-white">
      <div className="px-6 py-3">
        ...
        <div className="mt-2 text-xs text-slate-500">
          {subTabConfig[activeSubTab]?.description}
        </div>
      </div>
    </div>
  );

...
function processMainLines(gameOdds: MLBGameOdds, parsed: any, bestOdds: OddsData, odd: DatabaseOdd) {
...
function processPlayerProps(gameOdds: MLBGameOdds, parsed: any, bestOdds: OddsData, odd: DatabaseOdd) {
...
function processTeamProps(gameOdds: MLBGameOdds, parsed: any, bestOdds: OddsData, odd: DatabaseOdd) {
  ...
  let teamProp = gameOdds.teamProps.find(
    p => p.team === team && p.market === displayName
  );
...
function processGameProps(gameOdds: MLBGameOdds, parsed: any, bestOdds: OddsData, odd: DatabaseOdd) {
  const { displayName, side } = parsed;
  
  // Find or create game prop
  let gameProp = gameOdds.gameProps.find(
    p => p.market === displayName
  );
...
function derivePlayerName(playerId: string): string {
  ...
  const nameParts = cleanId.split('_').map(part => 

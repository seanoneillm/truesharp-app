...
export function calculateSubscriptionPerformance(
  ...
  const subscriptionBets = userBets.filter(bet => 
    bet.source_strategy_id === subscription.strategy_id
  )
  ...
  const monthsSubscribed = Math.max(1, Math.ceil((now.getTime() - subscriptionStartDate.getTime()) / (30 * 24 * 60 * 60 * 1000)))
...
function calculateMonthlyBreakdown(
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const monthBets = userBets.filter(bet => {
      ...
      return betDate >= monthStart && betDate <= monthEnd && bet.is_copy_bet && bet.source_strategy_id
    })
    ...
    const monthROI = monthlySubscriptionsCost > 0 ? ((monthProfit - monthlySubscriptionsCost) / monthlySubscriptionsCost) * 100 : 0
...
export function calculateSubscriptionValue(
  ...
  const costEfficiency = monthlyCost > 0 && performance.netValue > 0 ? performance.netValue / monthlyCost : 0

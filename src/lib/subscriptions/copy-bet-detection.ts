...
export function detectCopyBet(
  ...
  let teamMatch = true
  if (originalBetData.home_team && originalBetData.away_team && userBet.home_team && userBet.away_team) {
    teamMatch = (
  ...
  const placedAfterOriginal = userTime > originalTime + (COPY_BET_TOLERANCE.minTimeDifference * 60 * 1000)
...
export function batchDetectCopyBets(
  ...
  const uniqueMatches = matches.reduce((acc, match) => {
    ...
    if (!existingMatch) {
      acc.push(match)
    } else if (getConfidenceScore(match.confidence) > getConfidenceScore(existingMatch.confidence)) {
...
export function shouldExcludeFromMonetization(
  bet: {
    is_copy_bet: boolean
    source_strategy_id?: string
  }

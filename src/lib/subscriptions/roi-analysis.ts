...
function calculateSubscriptionCosts(subscriptions: Subscription[], timeframe: string): number {
  ...
  return subscriptions.reduce((total, subscription) => {
    ...
    cost = dailyRate * Math.min(timeframeDays, Math.ceil((endDate.getTime() - startDate.getTime()) / (24 * 60 * 60 * 1000)))
...
function calculateConfidenceInterval(
  ...
  const avgOdds = settledBets.reduce((sum, bet) => sum + Math.abs(bet.potential_payout - bet.stake), 0) / settledBets.length
  ...
  const lowerROI = totalCost > 0 ? ((lowerWinRate * avgOdds * bets.length - totalCost) / totalCost) * 100 : 0
  const upperROI = totalCost > 0 ? ((upperWinRate * avgOdds * bets.length - totalCost) / totalCost) * 100 : 0
...
function generateQuarterlyROI(
  monthlyData: Array<{ month: string; roi: number; profit: number; cost: number; netProfit: number }>
...
function projectFutureROI(
  ...
  const avgROI = recentPerformance.reduce((sum, month) => sum + month.roi, 0) / recentPerformance.length
  const avgProfit = recentPerformance.reduce((sum, month) => sum + month.profit, 0) / recentPerformance.length
  const avgCost = recentPerformance.reduce((sum, month) => sum + month.cost, 0) / recentPerformance.length
...
function generateROIRecommendations(
  ...
  subscriptions.forEach(subscription => {
    const subscriptionBets = userBets.filter(bet => bet.source_strategy_id === subscription.strategy_id)

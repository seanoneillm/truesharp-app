...
export function classifyMainTab(oddID: string): 'main' | 'player-props' | 'team-props' | 'game-props' {
  ...
  if (category === 'points' && (target === 'home' || target === 'away') && 
...
function getMLBSubTab(category: string, mainTab: string): string {
  if (mainTab === 'Player Props') {
    if (category.startsWith('batting_') || category === 'points' || category === 'fantasyScore' || 
        category === 'firstToScore' || category === 'lastToScore') {
      return 'Hitters';
    }
...
function getMLBSubSubTab(category: string, mainTab: string, subTab: string): string {
  if (mainTab === 'Player Props' && subTab === 'Hitters') {
    if (category === 'batting_hits' || category === 'batting_homeRuns' || 
        category === 'batting_RBI' || category === 'batting_totalBases' ||
        category === 'batting_singles' || category === 'batting_doubles' || 
...
function getNFLSubTab(category: string, mainTab: string): string {
  if (mainTab === 'Player Props') {
    ...
    if (category.startsWith('kicking_') || category.startsWith('fieldGoals_') || 
        category.startsWith('extraPoints_') || category.startsWith('defense_')) {
...
function getNBASubTab(category: string, mainTab: string): string {
  if (mainTab === 'Player Props') {
    if (category === 'points' || category.startsWith('fieldGoals_') || 
        category.startsWith('threePointers_') || category.startsWith('freeThrows_')) {
      return 'Scoring';
    }
    if (category.startsWith('rebounds')) {
      return 'Rebounding';
    }
    if (category === 'assists' || category === 'turnovers' || 
        category === 'steals' || category === 'blocks') {
      return 'Playmaking';
    }
    if (category.includes('+') || category === 'doubleDouble' || 
        category === 'tripleDouble' || category === 'fantasyScore') {
      return 'Combo Props';
    }
  }
...
function getNHLSubTab(category: string, mainTab: string): string {
  if (mainTab === 'Player Props') {
    if (category === 'goals' || category === 'assists' || category === 'points' ||
        category === 'shots_onGoal' || category === 'hits' || category === 'blockedShots') {
      return 'Skaters';
    }
    if (category === 'saves' || category === 'goalsAgainst' || 
        category === 'savePercentage' || category === 'shutout') {
      return 'Goalies';
    }
  }
...
function getSoccerSubTab(category: string, mainTab: string): string {
  if (mainTab === 'Player Props') {
    if (category === 'goals' || category === 'shots' || category === 'shots_onTarget' ||
        category === 'assists') {
      return 'Forwards';
    }
    if (category === 'passesCompleted' || category === 'passCompletionPercentage' ||
        category === 'tackles') {
      return 'Midfielders';
    }
...
export function organizeOddsByHierarchy(odds: DatabaseOdds[], sport: string): HierarchicalOdds {
  ...
  console.log(`📊 Filtered to ${validOdds.length} valid odds from ${odds.length} total odds (excluded yn markets)`);
...
export function getAvailableTabs(organizedOdds: Record<string, Record<string, Record<string, DatabaseOdds[]>>>, sport?: string) {
  ...
  const getDefaultSubTabs = (mainTab: string, sport?: string): string[] => {
    ...
    switch (sport.toLowerCase()) {
      ...
      case 'americanfootball_nfl':
      case 'americanfootball_ncaaf':
        if (mainTab === 'player-props') return ['Quarterback', 'Running Back', 'Wide Receiver/Tight End', 'Kicker/Defense'];
        if (mainTab === 'team-props') return ['Team Totals'];
        if (mainTab === 'game-props') return ['Game Totals'];
        return ['Totals'];
      case 'basketball_nba':
      case 'basketball_ncaab':
        if (mainTab === 'player-props') return ['Scoring', 'Rebounding', 'Playmaking', 'Combo Props'];
        if (mainTab === 'team-props') return ['Team Totals'];
        if (mainTab === 'game-props') return ['Game Totals'];
        return ['Totals'];
      case 'icehockey_nhl':
      ...
      case 'soccer_uefa_champs_league':
      case 'soccer_usa_mls':
        if (mainTab === 'player-props') return ['Forwards', 'Midfielders', 'Defenders', 'Goalkeepers'];
        if (mainTab === 'team-props') return ['Team Totals'];
        if (mainTab === 'game-props') return ['Game Totals'];
        return ['Totals'];
      default:
  ...
  return {
    ...
    getSubSubTabs: (mainTab: string, subTab: string) => {
      const actualSubSubTabs = organizedOdds[mainTab]?.[subTab] ? Object.keys(organizedOdds[mainTab][subTab]) : [];

...
function createBookmakerFromOdds(sportsbookName: string, odds: DatabaseOdds[], gameData: GameWithOdds): Bookmaker {
...
function normalizeMarketName(marketName: string): string {
  ...
  if (normalized.includes('spread') || normalized.includes('point spread') || normalized.includes('run line')) {
    return 'spreads';
  }
  // Only convert to 'totals' if it's a team total, not player props
  if ((normalized.includes('total') || normalized.includes('over/under') || normalized.includes('ou')) && 
...
function convertOddsToMarket(marketName: string, odds: DatabaseOdds[], gameData: GameWithOdds): Market | null {
  ...
  if (marketName === 'h2h') {
    ...
    odds.forEach(odd => {
      if (odd.bookodds) {
        ...
        const isHomeTeam = odd.hometeam === gameData.home_team || 
                          odd.hometeam === gameData.home_team_name;
        const isAwayTeam = odd.awayteam === gameData.away_team || 
                          odd.awayteam === gameData.away_team_name;
    ...
    odds.forEach(odd => {
      if (odd.bookodds && (odd.line !== null || odd.closebookodds !== null)) {
        ...
        const isOver = isOverByOddId || (!isUnderByOddId && (odd.sideid === 'over' || odd.sideid === 'o'));
        const isUnder = isUnderByOddId || (!isOverByOddId && (odd.sideid === 'under' || odd.sideid === 'u'));
...
function groupOddsByOverUnder(odds: DatabaseOdds[]): Array<{isOver: boolean, odds: DatabaseOdds}> {
  return odds.map(odd => {
    // Check oddID first (most reliable), then fallback to other methods
    const isOverByOddId = odd.oddid && odd.oddid.includes('-ou-over');
    const isOver = isOverByOddId || odd.sideid === 'over' || odd.marketname.toLowerCase().includes(' over');

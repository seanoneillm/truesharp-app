...
export function getTabForMarket(marketName: string, oddId?: string): string {
  ...
  if (oddId) {
    ...
    if (oddIdLower.includes('points-home-game-ml-') || 
    ...
    if (oddIdLower.includes('-any_player_id-') || 
        oddIdLower.match(/-(batting|pitching|passing|rushing|receiving|defense|fieldgoals|extrapoints|kicking|assists|rebounds|steals|blocks|fantasyscore|turnovers|saves|shotsongoal|goals|points|hits|penaltyminutes|powerpaypoints|blockedshots)-[^-]+-game-/)) {
      return 'player-props';
    }

    // Team Props patterns: home/away specific markets (not main lines)
    if ((oddIdLower.includes('-home-game-') || oddIdLower.includes('-away-game-')) &&
    ...
    if (oddIdLower.includes('-all-game-') && 
        !oddIdLower.includes('points-all-game-ou-') &&
...
export function getMLBPlayerPropsSubTab(marketName: string, oddId?: string): string {
  ...
  if (oddId) {
    ...
    if (oddIdLower.includes('pitching_strikeouts-') || 
    ...
    if (oddIdLower.includes('batting_hits-') ||
  ...
  const pitcherTerms = [
    'strikeouts', 'strikeout', 'k', 'earned_runs', 'era', 'hits_allowed', 
    'walks_allowed', 'home_runs_allowed', 'pitches_thrown', 'outs_recorded', 
    'pitching_win', 'saves', 'whip', 'innings_pitched', 'complete_games',
    'shutouts', 'balks', 'wild_pitches'
  ];
  
  // Hitting-specific terms
  const hitterTerms = [
    'hits', 'hit', 'rbi', 'runs', 'doubles', 'singles', 'triples', 
    'home_runs', 'hr', 'stolen_bases', 'sb', 'total_bases', 'walks', 
    'bb', 'batting_average', 'at_bats', 'sacrifice_flies', 'gidp'
  ];
  ...
  const knownPitchers = [
    'shohei_ohtani', 'gerrit_cole', 'spencer_strider', 'jacob_degrom',
    'shane_bieber', 'corbin_burnes', 'brandon_woodruff'
  ];
  
  // Known hitters (you can expand this list based on your data)
  const knownHitters = [
    'aaron_judge', 'mookie_betts', 'freddie_freeman', 'vladimir_guerrero',
    'ronald_acuna', 'mike_trout', 'manny_machado'
  ];
...
export function getNFLPlayerPropsSubTab(marketName: string, oddId?: string): string {
  ...
  if (oddId) {
    ...
    if (oddIdLower.includes('passing_yards-') || 
    ...
    if (oddIdLower.includes('rushing_yards-') ||
    ...
    // Receiver patterns  
    if (oddIdLower.includes('receiving_yards-') ||
    ...
    if (oddIdLower.includes('kicking_totalpoints-') ||
    ...
    // Any Player patterns
    if (oddIdLower.includes('fantasyscore-any_player_id-') ||
  ...
  if (defenseTerms.some(term => normalized.includes(term)) || kickerTerms.some(term => normalized.includes(term))) return 'kicker-defense';
...
export function getNBAPlayerPropsSubTab(marketName: string, oddId?: string): string {
  ...
  if (oddId) {
    ...
    // Scoring patterns
    if (oddIdLower.includes('points-any_player_id-') || 
    ...
    // Rebounding patterns
    if (oddIdLower.includes('rebounds_total-any_player_id-') ||
    ...
    // Playmaking patterns  
    if (oddIdLower.includes('assists-any_player_id-') ||
    ...
    if (oddIdLower.includes('points+rebounds-any_player_id-') ||
    ...
    // Special patterns
    if (oddIdLower.includes('doubledouble-any_player_id-') ||
...
export function getNHLPlayerPropsSubTab(marketName: string, oddId?: string): string {
  ...
  if (oddId) {
    ...
    // Goalie patterns
    if (oddIdLower.includes('saves-any_player_id-') || 
    ...
    if (oddIdLower.includes('points-any_player_id-') || // goals
...
function isMainLineMarket(marketName: string): boolean {
  ...
  if (marketName.includes('-home-game-ml') ||
  ...
  if ((marketName.includes('total') || marketName.includes('over/under') || marketName.includes('ou')) &&
...
function isPlayerPropsMarket(marketName: string): boolean {
  ...
  const apiPlayerPatterns = [
    'batting_', 'pitching_', 'passing_', 'rushing_', 'receiving_',
    'fieldGoals_', 'threePointers_', 'freeThrows_', 'rebounds_',
    'shots_onGoal', 'penalty', 'power', 'fantasy'
  ];
  ...
  if (marketName.includes('-game-ou') || 
    ...
    return !marketName.includes('home-game-') && 
  ...
  const baseballPlayerTerms = [
    'hits', 'rbi', 'runs', 'doubles', 'home_run', 'hr', 'strikeouts', 'k',
    'walks', 'bb', 'stolen_bases', 'sb', 'total_bases', 'singles', 'triples',
    'earned_runs', 'era', 'pitch', 'saves', 'whip', 'batting_avg'
  ];
  ...
  const footballPlayerTerms = [
    ...
    'field_goals', 'sacks', 'tackles'
  ];
  
  const basketballPlayerTerms = [
    'points', 'rebounds', 'assists', 'steals', 'blocks', 'turnovers',
    'field_goals', 'three_pointers', 'free_throws', 'double_double', 'triple_double'
  ];
  
  const hockeyPlayerTerms = [
    'goals', 'assists', 'shots_on_goal', 'hits', 'blocked_shots',
    'penalty_minutes', 'power_play_points', 'saves', 'goals_against'
  ];
  
  const soccerPlayerTerms = [
    'shots', 'shots_on_target', 'fouls', 'cards', 'corners',
    'passes', 'tackles', 'clearances', 'interceptions'
  ];
  
  const allPlayerTerms = [
    ...baseballPlayerTerms, ...footballPlayerTerms, ...basketballPlayerTerms,
    ...hockeyPlayerTerms, ...soccerPlayerTerms
  ];
  ...
  return (hasPlayerPattern || hasPlayerStat) && 
...
function isTeamPropsMarket(marketName: string): boolean {
  ...
  if (marketName.includes('team_total') ||
  ...
  if (marketName.includes('points-home-game-ou') ||
  ...
  const teamTerms = [
    'team_to_', 'home_team_', 'away_team_', 'first_to_score', 'last_to_score',
    'firstScorer', 'lastScorer', 'teamToScore', 'cleanSheet'
  ];
...
function isGamePropsMarket(marketName: string): boolean {
  ...
  if (marketName.includes('game_total') ||
  ...
  // Game event patterns
  if (marketName.includes('overtime') ||
  ...
  if (marketName.includes('points-all-game-ou') ||
  ...
  const gameWideTerms = [
    'game_total', 'both_teams', 'combined_', 'total_game_',
    'game_even_odd', 'game_eo', 'gameTotal', 'totalGame'
  ];
...
function isPeriodPropsMarket(marketName: string): boolean {
  return marketName.includes('first_half') ||
         ...
         marketName.includes('inning_') && (marketName.includes('1') || marketName.includes('2') || marketName.includes('3'));
}

function isAltLinesMarket(marketName: string): boolean {
  return marketName.includes('alt_spread') ||

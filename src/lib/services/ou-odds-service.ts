...
function subjectToEntity(subject: string): {
  entityType: 'player' | 'team' | 'game'
  entityId?: string
} {
...
> = {
  ...
  passing_longestCompletion: {
    row1: 'Player Props',
    row2: 'Quarterback',
    row3: 'Longest Completion',
  },
  'passing+rushing_yards': {
    row1: 'Player Props',
    row2: 'Quarterback',
    row3: 'Passing + Rushing Yards',
  },
  ...
  receiving_yards: {
    row1: 'Player Props',
    row2: 'Wide Receiver/Tight End',
    row3: 'Receiving Yards',
  },
  ...
  'rushing+receiving_yards': {
    row1: 'Player Props',
    row2: 'Running Back',
    row3: 'Rushing + Receiving Yards',
  },
  receiving_longestReception: {
    row1: 'Player Props',
    row2: 'Wide Receiver/Tight End',
    row3: 'Longest Reception',
  },
  ...
  defense_fumbleRecoveries: {
    row1: 'Player Props',
    row2: 'Kicker/Defense',
    row3: 'Fumble Recoveries',
  },
...
function getMetricMap(league: string): Record<
  string,
  {
    row1: string
    row2: string
...
export class OUOddsService {
  ...
  private filterToOUOdds(odds: DatabaseOdds[]): DatabaseOdds[] {
    return odds.filter(odd => {
      ...
      return (
        odd.oddid.includes('-ou-') && (odd.oddid.endsWith('-over') || odd.oddid.endsWith('-under'))
      )
    })
  }
  ...
  private normalizeOUOdd(odd: DatabaseOdds, league: string): NormalizedOUOdd | null {
    try {
      ...
      if (line === null || price === null) {
        console.warn(
          `⚠️ Invalid line (${odd.line}) or price (${odd.bookodds}) for oddID: ${odd.oddid}`
        )
        return null
      }
   ...
   */
  async getOUOddsForDate(
    date: string,
    leagues: string[] = ['MLB', 'NFL', 'NBA', 'NHL']
  ): Promise<Record<string, NormalizedOUOdd[]>> {
    try {
      ...
      for (const game of games) {
        ...
        console.log(
          `  Game ${game.id} (${game.league}): ${normalizedOdds.length} normalized O/U odds`
        )
      }

...
export class MLBOddsAnalyzer {
  ...
  async getMLBGamesWithOrganizedOdds(): Promise<MLBGameData[]> {
    try {
      ...
      const oddsByGameId = (allOdds || []).reduce(
        (acc, odd) => {
          if (!acc[odd.eventid]) {
            acc[odd.eventid] = []
          }
      ...
      mlbGameData.forEach(gameData => {
        ...
        console.log(
          `    Main Lines: ML(${organizedOdds.mainLines.moneyline.length}) RL(${organizedOdds.mainLines.runLine.length}) Tot(${organizedOdds.mainLines.totals.length})`
        )
        console.log(
          `    Player Props: Hit(${organizedOdds.playerProps.hitting.length}) Pitch(${organizedOdds.playerProps.pitching.length}) Gen(${organizedOdds.playerProps.general.length})`
        )
        console.log(
          `    Team Props: Tot(${organizedOdds.teamProps.teamTotals.length}) Adv(${organizedOdds.teamProps.advanced.length})`
        )
        console.log(
          `    Game Props: Flow(${organizedOdds.gameProps.gameFlow.length}) Special(${organizedOdds.gameProps.special.length})`
        )
  ...
  private isPlayerProp(marketName: string, playerId: string | null): boolean {
    return (
      ...
      (marketName.includes('_') && marketName.length > 15)
    ) // Likely player name
  }
   ...
   */
  async getOddsCategoryForGame(
    gameId: string,
    category: 'main' | 'player-props' | 'team-props' | 'game-props'
  ): Promise<DatabaseOdds[]> {
    try {

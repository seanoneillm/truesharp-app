...
export class GamesDataService {
  ...
  async getMLBGameWithOdds(gameId: string): Promise<GameWithOdds | null> {
    try {
      ...
      console.log(
        `✅ Found ${odds?.length || 0} total odds, ${validOdds.length} valid odds (excluded yn) for game ${gameId}`
      )
      console.log(`📊 Odds breakdown:`, {
        ...
        other: validOdds.filter(
          (o: DatabaseOdds) =>
            !o.oddid?.includes('-ml-') && !o.oddid?.includes('-sp-') && !o.oddid?.includes('-ou-')
        ).length,
      })
  ...
  async getMLBGamesForDate(date: string): Promise<GameWithOdds[]> {
    try {
      ...
      const oddsByGameId = (odds || []).reduce(
        (acc, odd) => {
          if (!acc[odd.eventid]) {
            acc[odd.eventid] = []
          }
  ...
  async getGamesForLeagueAndDate(league: string, date: string): Promise<GameWithOdds[]> {
    try {
      ...
      console.log(
        `🕐 ${league} Eastern time range: ${startTime.toISOString()} to ${endTime.toISOString()}`
      )
      ...
      const oddsByGameId = validOdds.reduce(
        (acc, odd) => {
          if (!acc[odd.eventid]) {
            acc[odd.eventid] = []
          }
      ...
      console.log(
        `✅ Found ${games.length} ${league} games with ${odds?.length || 0} total odds, ${validOdds.length} valid odds`
      )
      console.log(
        `📊 Valid odds distribution by game:`,
        Object.keys(oddsByGameId).map(gameId => ({
          gameId,
          oddsCount: oddsByGameId[gameId].length,
          breakdown: {
            moneyline: oddsByGameId[gameId].filter((o: DatabaseOdds) => o.oddid?.includes('-ml-'))
              .length,
            spread: oddsByGameId[gameId].filter((o: DatabaseOdds) => o.oddid?.includes('-sp-'))
              .length,
            overUnder: oddsByGameId[gameId].filter((o: DatabaseOdds) => o.oddid?.includes('-ou-'))
              .length,
            other: oddsByGameId[gameId].filter(
              (o: DatabaseOdds) =>
                !o.oddid?.includes('-ml-') &&
                !o.oddid?.includes('-sp-') &&
                !o.oddid?.includes('-ou-')
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      const { error } = await this.supabase.from('games').select('count').limit(1)
  ...
  async getDataSummary(): Promise<{
    ...
    try {
      ...
      const gamesByLeague = (gamesData || []).reduce(
        (acc, game) => {
      ...
      const gamesAvailableDates = Array.from(
        new Set((gamesData || []).map(game => game.game_time.split('T')[0]))
      ).sort()

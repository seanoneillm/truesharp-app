...
function determineBetType(marketType: string): DatabaseBet['bet_type'] {
  ...
  if (
    lowerMarket.includes('-sp-') ||
    lowerMarket.includes('spread') ||
    lowerMarket.includes('line')
  ) {
    return 'spread'
  }
  if (
    lowerMarket.includes('-ou-') ||
    lowerMarket.includes('total') ||
...
function extractPropType(marketType: string): string | undefined {
  ...
  if (lowerMarket.includes('batting_homeruns') || lowerMarket.includes('batting_homerun'))
    return 'home_runs'
...
export function mapBetToDatabase(
  ...
  return {
    ...
    profit: 0, // Profit should be 0 for pending bets, especially parlays
    ...
    is_parlay: isParlay,
    ...(propType && { prop_type: propType }),
    ...(playerName && { player_name: playerName }),
    ...(bet.line !== undefined && { line_value: bet.line }),
    ...(side && { side: side }),
    ...(parlayId && { parlay_id: parlayId }),
  }
}

...
export async function fetchGamesForSport(sportKey: string, date?: Date): Promise<Game[]> {
  try {
    ...
    if (!API_KEY || API_KEY === 'demo-key') {
      console.warn('Odds API key not configured. Add NEXT_PUBLIC_ODDS_API_KEY to your environment variables.');
...
function getTeamSpecificPlayers(homeTeam: string, awayTeam: string, sportKey: string): Array<{name: string, position: string, team: string}> {
...
function getTeamRostersBySport(sportKey: string): Record<string, Array<{name: string, position: string, team: string}>> {
  switch (sportKey) {
    case 'basketball_nba':
      return {
        'Los Angeles Lakers': [
...
function getGenericPlayersBySport(sportKey: string): Array<{name: string, position: string, team: string}> {
...
function getPropTypesForSport(sportKey: string): string[] {
  const propsBySort: Record<string, string[]> = {
    'basketball_nba': [
      ...
      'Turnovers', 'Free Throws Made', 'Points + Rebounds + Assists', 'Double-Double'
    ],
    'americanfootball_nfl': [
      'Passing Yards', 'Passing TDs', 'Interceptions', 'Completions', 'Rushing Yards', 
      'Rushing TDs', 'Receiving Yards', 'Receptions', 'Receiving TDs', 'Longest Reception'
    ],
    'baseball_mlb': [
      'Hits', 'Home Runs', 'RBIs', 'Runs Scored', 'Total Bases', 'Stolen Bases',
      'Strikeouts (Pitcher)', 'Walks', 'Earned Runs', 'Innings Pitched'
    ],
    'icehockey_nhl': [
      'Goals', 'Assists', 'Points', 'Shots on Goal', 'Penalty Minutes',
      'Saves', 'Goals Against', 'Save Percentage', 'Time on Ice'
    ],
...
function getGamePropsForSport(sportKey: string): Array<{type: string, description: string, options: Array<{name: string, basePrice: number}>}> {
  const propsBySort: Record<string, Array<{type: string, description: string, options: Array<{name: string, basePrice: number}>}>> = {
    'basketball_nba': [
      {
        type: 'first_basket',
...
function getFuturesForSport(sportKey: string): Array<{type: string, description: string, options: Array<{name: string, basePrice: number}>}> {
  const futuresBySort: Record<string, Array<{type: string, description: string, options: Array<{name: string, basePrice: number}>}>> = {
    'basketball_nba': [
      {
        type: 'championship',

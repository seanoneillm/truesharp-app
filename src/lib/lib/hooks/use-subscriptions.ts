...
interface UseSubscriptionsReturn {
  ...
  upgradeSubscription: (subscriptionId: string, newTier: 'bronze' | 'silver' | 'premium') => Promise<boolean>
...
export function useSubscriptions(): UseSubscriptionsReturn {
  ...
  const subscribe = useCallback(async (sellerId: string, tier: 'bronze' | 'silver' | 'premium'): Promise<boolean> => {
    const response = await authenticatedRequest(async (currentUserId) => {
      ...
      const price = tier === 'bronze' ? sellerSettings.bronze_price :
                   tier === 'silver' ? sellerSettings.silver_price :
  ...
  const resumeSubscription = useCallback(async (subscriptionId: string): Promise<boolean> => {
    ...
    if (response.success) {
      setSubscriptions(prev => 
        prev.map(sub => 
          sub.id === subscriptionId 
            ? { ...sub, status: 'active', expires_at: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() }
            : sub
        )
      )
      return true
  ...
  const upgradeSubscription = useCallback(async (subscriptionId: string, newTier: 'bronze' | 'silver' | 'premium'): Promise<boolean> => {
    const response = await authenticatedRequest(async (currentUserId) => {
      ...
      const newPrice = newTier === 'bronze' ? sellerSettings.bronze_price :
                      newTier === 'silver' ? sellerSettings.silver_price :
                      sellerSettings.premium_price
  ...
  const getSubscriptionBySeller = useCallback((sellerId: string): SubscriptionWithSeller | null => {
    return subscriptions.find(sub => sub.seller_id === sellerId && sub.status === 'active') || null
  }, [subscriptions])

  // Check if subscribed to a seller
  const isSubscribedTo = useCallback((sellerId: string): boolean => {
    return subscriptions.some(sub => sub.seller_id === sellerId && sub.status === 'active')
  }, [subscriptions])

  // Check if can access a specific tier
  const canAccessTier = useCallback((sellerId: string, tier: 'free' | 'bronze' | 'silver' | 'premium'): boolean => {

...
export async function GET(
  ...
  try {
    ...
    if (strategy && strategy.user_id) {
      ...
      const targetStrategy = strategiesWithBets.find((s: any) => s.id === strategyId);
      console.log('Target strategy found:', targetStrategy);
      
      if (targetStrategy && (targetStrategy as any).open_bets) {
        // Format to match expected structure for modal
        openBets = (targetStrategy as any).open_bets.map((bet: any) => ({
          id: `shared-${bet.id}`,
          added_at: null,
          bet_id: bet.id,
          bets: bet

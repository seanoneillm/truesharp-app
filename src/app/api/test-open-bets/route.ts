  ...
  try {
    ...
    if (testType === 'seller') {
      ...
      summary = {
        ...
        strategiesWithOpenBets: result.filter((s: unknown) => (s as { open_bets_count: number }).open_bets_count > 0).length,
        totalOpenBets: result.reduce((sum: number, s: unknown) => sum + ((s as { open_bets_count?: number }).open_bets_count || 0), 0),
        totalPotentialProfit: result.reduce((sum: number, s: unknown) => sum + ((s as { total_potential_profit?: number }).total_potential_profit || 0), 0)
      }
    ...
    return NextResponse.json({
      ...
      userId,
      summary,
      strategiesWithOpenBets: result.map((s: unknown) => {
        const strategy = s as { 
          id: string; 
          name: string; 
        ...
        return {
          id: strategy.id,
          name: strategy.name,
          open_bets_count: strategy.open_bets_count,
          total_potential_profit: strategy.total_potential_profit,
          sample_open_bets: strategy.open_bets?.slice(0, 2).map((bet: unknown) => {
            const betObj = bet as { 
              id: string; 
            ...
            return {
              id: betObj.id,
              bet_description: betObj.bet_description,
              odds: betObj.odds,
              stake: betObj.stake,
              potential_payout: betObj.potential_payout,

  ...
  try {
    ...
    if (testType === 'seller') {
      ...
      summary = {
        ...
        strategiesWithOpenBets: result.filter(
          (s: unknown) => (s as { open_bets_count: number }).open_bets_count > 0
        ).length,
        totalOpenBets: result.reduce(
          (sum: number, s: unknown) =>
            sum + ((s as { open_bets_count?: number }).open_bets_count || 0),
          0
        ),
        totalPotentialProfit: result.reduce(
          (sum: number, s: unknown) =>
            sum + ((s as { total_potential_profit?: number }).total_potential_profit || 0),
          0
        ),
      }

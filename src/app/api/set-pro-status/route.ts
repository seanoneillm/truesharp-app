...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      ...
      if (userId) {
        console.log('Set Pro Status API - Session auth failed, using service role with userId:', userId.substring(0, 8) + '...')
        ...
        if (!['yes', 'no'].includes(proStatus)) {
          return NextResponse.json({ error: 'Invalid pro status. Must be "yes" or "no"' }, { status: 400 })
        }
        
        console.log('Set Pro Status API - Updating pro status to:', proStatus, 'for user:', effectiveUserId.substring(0, 8) + '...')
        ...
        return NextResponse.json({ error: 'Unauthorized - no user session or userId provided' }, { status: 401 })
      }
    }

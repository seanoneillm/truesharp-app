...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: pendingBets, error } = await supabase
      .from('bets')
      .select('id, sport, league, bet_type, bet_description, odds, stake, potential_payout, game_date, home_team, away_team, player_name, prop_type, line_value, side, sportsbook, placed_at')
      .eq('user_id', user.id)
      .eq('status', 'pending')

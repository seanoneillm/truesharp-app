...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userId !== user.id) {
      ...
      return NextResponse.json(
        { valid: false, error: 'User ID mismatch' },
        { status: 403 }
      );
    }
    ...
    const matchingTransaction = receiptInfo.find(transaction => 
    ...
    if (matchingTransaction.product_id !== productId) {
      ...
      return NextResponse.json(
        { valid: false, error: 'Product ID mismatch' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { valid: false, error: 'Unexpected server error' },
      { status: 500 }
    );
  }
}
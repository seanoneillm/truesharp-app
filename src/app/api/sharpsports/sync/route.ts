...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      ...
      if (userId) {
        console.log('SharpSports Sync - Using service role with userId:', userId.substring(0, 8) + '...')
    ...
    for (const account of accounts) {
      try {
        ...
        const betsProcessed = await syncBetSlipsForAccount(querySupabase, account.bettor_id, effectiveUserId!)
        ...
        console.error(`❌ SharpSports Sync - Account ${account.sharpsports_account_id} sync error:`, error)
...
async function syncBetSlipsForAccount(supabase: any, bettorId: string, userId: string): Promise<number> {
  ...
  try {
    ...
    console.log(`✅ SharpSports Sync - Completed syncing betSlips for bettor: ${bettorId}, processed ${totalBetsProcessed} bets`)
...
async function processBet(supabase: any, bet: any, betSlip: any, userId: string, parlayId: string | null): Promise<boolean> {
  try {

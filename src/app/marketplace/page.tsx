    ...
    <DashboardLayout>
        ...
        <div className="hidden lg:flex items-center space-x-8">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              +{strategies.length > 0 ? (strategies.reduce((sum, s) => sum + s.roi_percentage, 0) / strategies.length).toFixed(1) : '0'}%
            </div>
            <div className="text-sm text-slate-600">Average ROI</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {strategies.length > 0 ? (strategies.reduce((sum, s) => sum + s.win_rate, 0) / strategies.length).toFixed(1) : '0'}%
            </div>
      ...
      <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
        ...
        <select
          ...
          className="rounded-xl border border-slate-300 bg-white py-3 px-4 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 min-w-[140px]"
        >
        ...
        <select
          ...
          className="rounded-xl border border-slate-300 bg-white py-3 px-4 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 min-w-[160px]"
        >
      ...
      {!strategiesLoading && !error && filteredStrategies.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center">
            <span className="bg-gradient-to-r from-yellow-400 to-yellow-600 text-white px-3 py-1 rounded-full text-sm font-bold mr-3">
              TOP PERFORMERS
            </span>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-slate-600">
            Sorted by {sortBy === 'leaderboard' ? 'Leaderboard Ranking' : 
                      sortBy === 'roi' ? 'ROI Performance' :
                      sortBy === 'winRate' ? 'Win Rate' : 'Activity'}
          </div>
        </div>
        ...
        {strategiesLoading ? (
          ...
          <div className="p-6 text-center border border-red-200 bg-red-50 rounded-lg">
            <div className="text-red-600 font-medium mb-2">Failed to load strategies</div>
            <div className="text-red-500 text-sm">{error}</div>
          </div>
        ) : filteredStrategies.length === 0 ? (
          <div className="p-8 text-center border rounded-lg">
            <Store className="h-12 w-12 text-slate-400 mx-auto mb-4" />
            <div className="text-slate-600 font-medium mb-2">No strategies available</div>
            <div className="text-slate-500 text-sm">Check back later for new strategy listings</div>
          </div>
        ) : (
          <div className="space-y-4">

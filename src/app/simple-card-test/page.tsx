...
export default function SimpleCardTestPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-8">
        <h1 className="mb-4 text-2xl font-bold">Loading games...</h1>
  ...
  return (
    <div className="p-8">
      <h1 className="mb-6 text-2xl font-bold">Simple Card Test - MLB Games</h1>
      ...
      {games.length === 0 ? (
        ...
        <div className="space-y-6">
          {games.map(game => (
            <SimpleOddsCard key={game.id} game={game} />
          ))}
        </div>
      )}
    </div>

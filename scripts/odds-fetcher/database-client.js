...
export class SupabaseClient {
  constructor() {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      throw new Error(
        'NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required in .env file'
      )
    }
  ...
  async upsertOdds(oddsData) {
    ...
    try {
      ...
      for (let i = 0; i < oddsData.length; i += BATCH_SIZE) {
        ...
        console.log(
          `📦 Processing batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(oddsData.length / BATCH_SIZE)} (${batch.length} entries)`
        )
      ...
      // Log summary
      const lineCount = results.data.filter(
        odd => odd.line !== null && odd.line !== undefined
      ).length
      const scoreCount = results.data.filter(
        odd => odd.score !== null && odd.score !== undefined
      ).length
      console.log(
        `📊 Final summary: ${results.data.length} total inserted, ${lineCount} with line values, ${scoreCount} with scores`
      )
  ...
  async upsertGameAndOddsData(normalizedData) {
    ...
    try {
      ...
      if (results.overall_success) {
        ...
        console.log(
          `📊 Summary: ${results.games.data.length} games, ${results.odds.data.length} odds entries`
        )
      }
  ...
  async testConnection() {
    try {
      ...
      const { data, error } = await this.supabase.from('games').select('count').limit(1)
  ...
  async cleanupOldData(daysOld = 30) {
    try {
      ...
      console.log(
        `🧹 Cleaning up data older than ${daysOld} days (before ${cutoffDate.toISOString()})`
      )

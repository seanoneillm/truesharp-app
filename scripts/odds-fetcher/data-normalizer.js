export class DataNormalizer {
  ...
  normalizeGameData(apiEvent) {
    try {
      ...
      console.log(`📋 Normalized game: ${normalizedGame.id} - ${normalizedGame.away_team_name} @ ${normalizedGame.home_team_name}`);
  ...
  normalizeOddEntry(odd, eventID) {
    return {
      ...
      // Score value
      score: odd.score !== undefined && odd.score !== null ? String(odd.score).substring(0, 50) : null,
  ...
  normalizeEventsBatch(apiEvents) {
    ...
    console.log(`✅ Normalized ${normalizedData.games.length} games and ${normalizedData.odds.length} odds entries`);

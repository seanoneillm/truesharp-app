...
export class OddsFetcher {
  ...
  async run(leagues = ['MLB'], startsAfter = '2025-08-14', startsBefore = '2025-08-15') {
    ...
    console.log(`🔧 Mode: ${this.testMode ? 'TEST (logging only)' : 'INSERT (will save to database)'}`);
  ...
  async fetchEventsByLeague(league, startsAfter, startsBefore) {
    ...
    if (!fetchFunction) {
      throw new Error(`Unsupported league: ${league}. Supported: ${Object.keys(leagueMap).join(', ')}`);
    }
  ...
  async showDatabaseSummary() {
    try {
      ...
      recentGames.forEach((game, index) => {
        console.log(`  ${index + 1}. ${game.away_team_name} @ ${game.home_team_name} (${game.status})`);
      });
  ...
  async fetchAllSports(startsAfter, startsBefore) {
    return this.run(['MLB', 'NBA', 'NFL', 'MLS', 'NHL', 'NCAAF', 'NCAAB', 'UCL'], startsAfter, startsBefore);
  }
}
...
async function main() {
  ...
  const startsAfter = startIndex !== -1 && args[startIndex + 1] ? args[startIndex + 1] : '2025-08-14';

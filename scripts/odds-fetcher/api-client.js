...
export class SportsGameOddsAPI {
  ...
  async fetchEvents(leagueID, startsAfter, startsBefore) {
    ...
    do {
      try {
        pageCount++
        console.log(
          `📄 Fetching page ${pageCount}${nextCursor ? ` (cursor: ${nextCursor.substring(0, 20)}...)` : ''}`
        )
  ...
  async fetchMultipleLeagues(leagues, startsAfter, startsBefore) {
    console.log(
      `📡 Fetching multiple leagues: ${leagues.join(', ')} from ${startsAfter} to ${startsBefore}`
    )
  ...
  async fetchAllSportsEvents(startsAfter, startsBefore) {
    return this.fetchMultipleLeagues(

-- Final Fixed SQL for existing pro_subscriptions table
-- Execute this in Supabase SQL editor

-- First, let's check what columns actually exist and add missing ones
DO $$ 
BEGIN
    -- Add missing transaction_validation_status column
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'pro_subscriptions' 
        AND column_name = 'transaction_validation_status'
    ) THEN
        ALTER TABLE pro_subscriptions 
        ADD COLUMN transaction_validation_status TEXT DEFAULT 'pending' 
        CHECK (transaction_validation_status IN ('pending', 'validated', 'failed'));
    END IF;

    -- Add missing indexes
    IF NOT EXISTS (
        SELECT 1 FROM pg_indexes 
        WHERE indexname = 'idx_pro_subscriptions_transaction_validation_status'
    ) THEN
        CREATE INDEX idx_pro_subscriptions_transaction_validation_status 
        ON pro_subscriptions(transaction_validation_status);
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_indexes 
        WHERE indexname = 'idx_pro_subscriptions_environment'
    ) THEN
        CREATE INDEX idx_pro_subscriptions_environment 
        ON pro_subscriptions(apple_environment);
    END IF;

    -- Add unique constraint on apple_transaction_id if not already present
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'pro_subscriptions_apple_transaction_id_key'
    ) THEN
        ALTER TABLE pro_subscriptions 
        ADD CONSTRAINT pro_subscriptions_apple_transaction_id_key 
        UNIQUE (apple_transaction_id);
    END IF;
END $$;

-- Create audit table for modern transaction attempts
...
-- FIXED: Updated function that uses the CORRECT column names from your existing table
...
BEGIN
  ...
  -- FIXED: Use correct column name 'apple_original_transaction_id'
  ...
  IF v_existing_subscription IS NOT NULL THEN
    ...
    UPDATE pro_subscriptions SET
      ...
      receipt_validation_status = 'validated', -- Use existing column name
  ...
  ELSE
    ...
    ELSE
      ...
      INSERT INTO pro_subscriptions (
        ...
        apple_original_transaction_id, -- Use correct column name
        apple_environment,
        receipt_validation_status, -- Use existing column name

...
async function testLongFieldInsert() {
  try {
    ...
    const { data: games, error: gameError } = await supabase.from('games').select('id').limit(1)
    ...
    const tests = [
      {
        ...
        testOdds: {
          ...
          sideid: 'home',
          oddid:
            'this-is-a-very-long-oddid-that-exceeds-fifty-characters-and-should-trigger-error-123',
          bookodds: -110,
    ...
    for (const test of tests) {
      ...
      const { data, error } = await supabase.from('odds').insert([test.testOdds]).select()

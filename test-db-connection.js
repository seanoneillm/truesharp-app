...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRyc29nYWZyeHBwdHN6eHlkeWNuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA3MjQ0OTQsImV4cCI6MjA2NjMwMDQ5NH0.STgM-_-9tTwI-Tr-gajQnfsA9cEZplw7W5uPWmn-SwA';
...
async function testConnection() {
  try {
    ...
    const { data: mlbGames, error: mlbError, count } = await supabase
      .from('games')
      .select('*', { count: 'exact' })
      .eq('league', 'MLB')
      .limit(5);
    ...
    if (mlbGames && mlbGames.length > 0) {
      ...
      mlbGames.forEach((game, index) => {
        console.log(`  ${index + 1}. ${game.away_team} @ ${game.home_team} (${game.game_time.split('T')[0]}) - ID: ${game.id}`);
      });
      ...
      if (odds && odds.length > 0) {
        ...
        odds.forEach((odd, index) => {
          console.log(`  ${index + 1}. ${odd.oddid} | ${odd.sportsbook} | ${odd.bookodds} | ${odd.marketname}`);
        });
      } else {
